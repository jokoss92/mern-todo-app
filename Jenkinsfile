...
pipeline {
    ...
    stages {
        ...
        stage('Deploy Image to Kubernetes') { 
            steps {
                sh'''cd mern-todo-app && sed -i "80d" frontend.yml'''
                sh'''cd mern-todo-app && sed -i "79 a \'\\'          image: $DOCKER_REGISTRY/$DOCKER_IMAGE_FRONTEND:${BUILD_NUMBER}" frontend.yml && sed -i "s/''//" frontend.yml'''
